...
export default async function page({ params }: { params: any }) {
  ...
  const res = await fetchData(
    `/blogs?filters[slug][$eq]=${slug}&populate=cover`
  );
        ...
        <section className='lg:w-4/6 mx-auto my-32'>
          {
            <div className='flex flex-col'>
              <div className='flex flex-col gap-4 mb-10'>
                ...
                <h1 className='line-clamp-2 leading-10 text-xl md:text-4xl font-semibold'>
                  {blogData.title}
                </h1>
              </div>
              <div className='w-full relative h-[400px]'>
                <Image
                  className=' object-cover  w-full object-covers mb-4 min-h-full aspect-video'
                  src={blogData.img}
                  alt='blogData image'
                  width={500}
                  height={500}
              ...
              <div className='mt-10'>
                <article className='prose lg:prose-xl max-w-full '>
                  <BlocksRenderer content={blogData.content} />
                </article>
              </div>
            </div>
          }
            ...
            {otherBlogs?.map((post: any) => (
              <div
                ...
                <div className='w-full'>
                  <Image
                    className=' object-cover max-h-5xl w-full object-covers mb-4 aspect-[16/14] min-h-full'
                    src={post.img}
                    alt='blog image'
                    width={500}
                    height={500}
                ...
                <div className='flex flex-col gap-4 px-5 py-4'>
                  ...
                  <h3 className='line-clamp-2 lg:line-clamp-2  text-xl md:text-2xl font-medium'>
                    {post.title}
                  </h3>
                  <Link
                    href={`/blogs/${post.slug}`}
                    className='flex items-center  w-fit gap-2 text-lg font-semibold text-[--primary_color] transition hover:scale-105'>
                    {" "}
