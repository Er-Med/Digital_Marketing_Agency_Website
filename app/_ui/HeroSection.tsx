...
import { getHeroSectionData } from "../_lib/data";
...
export default function HeroSection() {
 ...
 const [sectionData, setSectionData] = useState<HeroSection | null>(null)

 useEffect(() => {
  async function fetchHeaderData() {
   try {
    const data = await getHeroSectionData()
    setSectionData(data.data.attributes.HeroSection)
   } catch (err) {
     ...
     <div className="text-center ">
      <h1
       ...
       <span className="block xl:inline">
        <span className="mb-1 block">{sectionData.top_title}</span>
        {sectionData.middle_title}
       </span>
       <div className="mt-2 bg-white">
        <span className="relative mt-3 whitespace-nowrap text-[--primary_color]"><svg aria-hidden="true" viewBox="0 0 418 42"
         ...
         <span className="relative text-5xl md:text-7xl lg:text-8xl">{sectionData.highlighted_title}</span>
        </span>
       </div>
      </h1>
      ...
      <div className="mt-5 sm:mt-8 flex justify-center">
       <div className="">
        <Button text={sectionData.button.text} href="/contact" type={sectionData.button.type} />
       </div>
      </div>
     </div>
    </div>
